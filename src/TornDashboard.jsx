...
const ALL_RARITIES = ["Red", "Orange", "Yellow"];

const DEFAULT_OPENINGS_WEAPON_FILTERS = {
...
const DEFAULT_OPENINGS_ARMOR_FILTERS = {
  rarities: { Red: true, Orange: true, Yellow: true },
  bonusSearch: "",
  sortCol: "date",
  sortDir: "desc",
...
function AllOpeningsWeapons({ data, light }) {
  const [filters, setFilters] = useState(() => {
    try {
      const saved = localStorage.getItem("openingsFilters_weapons");
      if (saved) return { ...DEFAULT_OPENINGS_WEAPON_FILTERS, ...JSON.parse(saved) };
    } catch {}
    return DEFAULT_OPENINGS_WEAPON_FILTERS;
  });

  useEffect(() => {
    try { localStorage.setItem("openingsFilters_weapons", JSON.stringify(filters)); } catch {}
  }, [filters]);
  ...
  const handleSort = (col) => {
    setFilters(f => ({
      ...f,
      sortCol: col,
      sortDir: f.sortCol === col ? (f.sortDir === "asc" ? "desc" : "asc") : (col === "date" ? "desc" : "asc"),
    }));
  };
  const sortIcon = (col) => filters.sortCol === col ? (filters.sortDir === "asc" ? " ↑" : " ↓") : " ↕";
  ...
  const clearAll        = () => setFilters(DEFAULT_OPENINGS_WEAPON_FILTERS);
  ...
  const rarityStyle = {
    Red:    { bg: light ? "#fee2e2" : "#ef444430", color: "#dc2626" },
    Orange: { bg: light ? "#ffedd5" : "#fb923c30", color: "#ea580c" },
    Yellow: { bg: light ? "#fefce8" : "#facc1530", color: "#ca8a04" },
  };

  return (
    <div className="space-y-4">
      <Card title="Filters — All Openings (Weapons)" light={light}>
        <div className="flex flex-wrap gap-6">
          <div>
            <div className={`text-xs font-semibold uppercase mb-2 ${subtext}`}>Rarity</div>
            <div className="flex flex-col gap-1">
              {ALL_RARITIES.map(r => (
                <label key={r} className={`flex items-center gap-2 cursor-pointer text-sm ${light ? "text-gray-700" : "text-slate-300"}`}>
                  <input type="checkbox" checked={filters.rarities[r]} onChange={() => toggleRarity(r)} className="accent-indigo-600" />
                  <span className="inline-block px-2 py-0.5 rounded text-xs font-semibold"
                    style={{ backgroundColor: rarityStyle[r]?.bg, color: rarityStyle[r]?.color }}>
                    {r}
                  </span>
                  <span className={subtext}>({counts.rar[r]})</span>
                </label>
              ))}
            </div>
          </div>
          <div>
            <div className={`text-xs font-semibold uppercase mb-2 ${subtext}`}>Cache Type</div>
            <div className="flex flex-col gap-1">
              {CACHE_TYPES_WEAPON.map(c => (
                <label key={c} className={`flex items-center gap-2 cursor-pointer text-sm ${light ? "text-gray-700" : "text-slate-300"}`}>
                  <input type="checkbox" checked={filters.cacheTypes[c]} onChange={() => toggleCacheType(c)} className="accent-indigo-600" />
                  {c}
                  <span className={subtext}>({counts.ct[c]})</span>
                </label>
              ))}
            </div>
          </div>
          <div className="flex-1 min-w-[180px]">
            <div className={`text-xs font-semibold uppercase mb-2 ${subtext}`}>Bonus Search</div>
            <div className="relative">
              <input
                ...
                className={`w-full rounded-lg px-3 py-2 pr-8 text-sm border ${inputBg} focus:outline-none focus:ring-2 focus:ring-indigo-500`}
              />
              {filters.bonusSearch && (
                <button
                  onClick={() => setFilters(f => ({ ...f, bonusSearch: "" }))}
                  className={`absolute right-2 top-1/2 -translate-y-1/2 text-lg leading-none font-bold ${subtext} hover:opacity-70`}
                >×</button>
              )}
            </div>
          </div>
          <div className="flex flex-col justify-end gap-2">
            <button onClick={clearAll}
              className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-colors ${light ? "bg-gray-100 hover:bg-gray-200 text-gray-700" : "bg-slate-700 hover:bg-slate-600 text-slate-200"}`}>
              Clear All Filters
            </button>
            <div className={`text-xs font-medium ${light ? "text-gray-600" : "text-slate-300"}`}>
      ...
      <div className={`${light ? "bg-white border-gray-200" : "bg-slate-800 border-slate-700"} border rounded-xl overflow-hidden`}>
        <div className="overflow-auto max-h-[60vh]">
          <table className="w-full text-sm text-left">
            <thead className={`text-xs uppercase ${thCls} border-b sticky top-0 z-10`}>
              <tr>
                {[
                  { col: "rarity",    label: "Rarity"      },
                  { col: "item",      label: "Weapon Name" },
                  { col: "bonus1",    label: "Bonus 1"     },
                  { col: "bonus2",    label: "Bonus 2"     },
                  { col: "cacheType", label: "Cache Type"  },
                  { col: "date",      label: "Date"        },
                ].map(({ col, label }) => (
                  <th key={col}
                    className="py-2 px-3 cursor-pointer select-none hover:opacity-70 transition-opacity whitespace-nowrap"
            ...
            <tbody>
              {sorted.map((d, i) => (
                <tr key={`${d.timestamp}-${d.weaponName}-${i}`}
                  className={`border-b ${trHoverCls} transition-colors`}>
                  <td className="py-2 px-3">
                    <span className="inline-block px-2 py-0.5 rounded text-xs font-semibold"
                      style={{ backgroundColor: rarityStyle[d.rarity]?.bg, color: rarityStyle[d.rarity]?.color }}>
                      {d.rarity}
                    </span>
                  </td>
                  <td className={`py-2 px-3 font-medium ${light ? "text-gray-900" : "text-white"}`}>{d.weaponName || "—"}</td>
                  <td className={`py-2 px-3 ${light ? "text-gray-700" : "text-slate-300"}`}>{d.bonus1}</td>
                  <td className={`py-2 px-3 ${light ? "text-gray-700" : "text-slate-300"}`}>{d.bonus2}</td>
                  <td className={`py-2 px-3 ${subtext}`}>{d.cacheType || "—"}</td>
                  <td className={`py-2 px-3 ${light ? "text-gray-400" : "text-slate-500"} text-xs`}>{d.timestamp || "—"}</td>
                </tr>
              ))}
              {sorted.length === 0 && (
                <tr>
                  <td colSpan={6} className="py-12 text-center">
                    <EmptyState message="No results match your filters" light={light} />
                  </td>
                </tr>
              )}
            </tbody>
...
function AllOpeningsArmor({ data, light }) {
  const [filters, setFilters] = useState(() => {
    try {
      const saved = localStorage.getItem("openingsFilters_armor");
      if (saved) return { ...DEFAULT_OPENINGS_ARMOR_FILTERS, ...JSON.parse(saved) };
    } catch {}
    return DEFAULT_OPENINGS_ARMOR_FILTERS;
  });

  useEffect(() => {
    try { localStorage.setItem("openingsFilters_armor", JSON.stringify(filters)); } catch {}
  }, [filters]);
  ...
  const sortIcon    = (col) => filters.sortCol === col ? (filters.sortDir === "asc" ? " ↑" : " ↓") : " ↕";
  const toggleRarity = (r) => setFilters(f => ({ ...f, rarities: { ...f.rarities, [r]: !f.rarities[r] } }));
  const clearAll     = () => setFilters(DEFAULT_OPENINGS_ARMOR_FILTERS);
  ...
  return (
    <div className="space-y-4">
      <Card title="Filters — All Openings (Armor)" light={light}>
        <div className="flex flex-wrap gap-6">
          <div>
            <div className={`text-xs font-semibold uppercase mb-2 ${subtext}`}>Rarity</div>
            <div className="flex flex-col gap-1">
              {ALL_RARITIES.map(r => (
                <label key={r} className={`flex items-center gap-2 cursor-pointer text-sm ${light ? "text-gray-700" : "text-slate-300"}`}>
                  ...
                  <span className={subtext}>({counts[r]})</span>
                </label>
              ))}
            </div>
          </div>
            ...
            <tbody>
              {sorted.map((d, i) => (
                <tr key={`${d.timestamp}-${d.weaponName}-${i}`}
                  ...
                  <td className={`py-2 px-3 font-medium ${light ? "text-gray-900" : "text-white"}`}>{d.weaponName || "—"}</td>
                  <td className={`py-2 px-3 ${light ? "text-gray-700" : "text-slate-300"}`}>{d.bonus || "—"}</td>
                  <td className={`py-2 px-3 ${light ? "text-gray-400" : "text-slate-500"} text-xs`}>{d.timestamp || "—"}</td>
                </tr>
              ))}
              {sorted.length === 0 && (
...
function AllOpeningsTab({ data, light }) {
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap gap-2">
        <button onClick={() => setSubTab("weapons")}
          className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${subTab === "weapons" ? btnActive : btnInactive}`}>
          Weapons ({weaponData.length})
        </button>
        <button onClick={() => setSubTab("armor")}
          className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${subTab === "armor" ? btnActive : btnInactive}`}>
          Armor ({armorData.length})
        </button>
      </div>
      {subTab === "weapons"
        ? <AllOpeningsWeapons data={weaponData} light={light} />
        : <AllOpeningsArmor  data={armorData}  light={light} />}
    </div>
  );
}
